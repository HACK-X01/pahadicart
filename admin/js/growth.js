...
function renderGrowthAnalytics() {
  ...
  if (heatmapContainer) {
    if (!PahadiMockDB.colonyHeatmap || PahadiMockDB.colonyHeatmap.length === 0) {
      heatmapContainer.innerHTML = '<div style="text-align:center; padding:24px; color:var(--slate-400); font-size:12.5px;">Colony density data will appear automatically as real deliveries are completed.</div>';
    } else {
  ...
  if (couponContainer) {
    if (!PahadiMockDB.coupons || PahadiMockDB.coupons.length === 0) {
      couponContainer.innerHTML = '<div style="text-align:center; padding:24px; color:var(--slate-400); font-size:12.5px;">No active discount vouchers running. Create promotions in Growth Settings.</div>';
    } else {
